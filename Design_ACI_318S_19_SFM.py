...
def data_columns_table(self):
    ...
    for DC in DataColDesing:
        ...
        nsB = DC.nsB
        sst = DC.sst / cm
        registro = RegistroColumns(self.ui.tbl_data_design_columns, DC.EleTag, b, h, db, nbH, nbB, nsH, nsB,
                                   sst)
...
# Colmuns P-M design
def AsColumn(b, h, EleTag, cover, dst, fy, Beta1C, Pu_v, Mu_v, Sum_Mn_B, FactorColBeamStr):
    ro_min = 0.01
    ro_max = 0.06
    npts = 20
    ncom = 10
                ...
                for nbB in nbB_v:
                    ...
                    if ro >= ro_min:
                        ...
                        if np.all((Pu_v >= min(fiPn)) & (Pu_v <= max(fiPn))):
                            ...
                            Col_to_beam_str_ratio = 2*max(Mn_i)/Sum_Mn_B
...
for (Ele, EleForceD, EleForceDL, EleForceDLE) in zip(Elements, ElemnsForceD, ElemnsForceDL, ElemnsForceDLE):
    ...
    if ListNodes[Ele.Nod_ini, 1] == ListNodes[Ele.Nod_end, 1]:
        ...
        sst, nsB, nsH = AvColumn(EleTag, Vu, b, h, nbH, nbB, dst, Ast, Vc, db, fy, Nu_min)
        ...
        DataColDesing.append(ColDesing(Ele.EleTag, b, h, nbH, nbB, db, As, Pu_v, Mu_v, fiPn, fiMn, Mn_i, d,
                                       ...
                                       NUG1, NUG2, MUD1, MUD2, VUD1, VUD2, ColBeamStr))
...
for DC in DataColDesing:
    ...
    Delta_x = xe - xi
    Delta_y = ye - yi
    ax.text(xe + 0.05*Delta_y, ye + 0.10*Delta_y, r'{:.1f} '.format(DC.ColBeamStr), style='italic', fontsize=6,
                va='bottom', ha='left', multialignment="right", bbox=dict(boxstyle='round', fc="w", ec="k"))
ax.axis('equal')
